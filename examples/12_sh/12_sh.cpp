...
class MeshBaker
{
    ...
    private:
        bool execute_impl(Context& context, RenderStage) override
        {
            if (executed_) return true;
            context.driver.set_render_state(*render_state_);
            context.driver.clear(color_, depth_, stencil_);
            executed_ = true;
            return true;
        }
...
private:
    ...
    sh::ColorSH render(size_t iteration, TextureBuffer& texture_buffer, DrawCallExplicit& draw_call, const vec3& x, const vec3& y, const vec3& z, const vec3& pos)
    {
        ...
        default_rasterizer_state.cull = cull_none;
...
#endif
                render_state_->update_rasterizer(skybox_rasterizer_state);
