...
layout(rgba8, binding = 0) readonly uniform image3D input_texture;
layout(rgba8, binding = 1) writeonly uniform image3D output_texture;
...
vec4 scattering(vec4 current, ivec3 cell_coord)
{
    ...
    return vec4(color, saturate(exp(-(scattering_coeff + next_value.a)))); // out scattering
}
...
void main()
{
    ...
    for (int i = 1; i < depth; ++i)
    {
        ...
        imageStore(output_texture, next_coord, next);
