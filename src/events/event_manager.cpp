...
void EventManager::check(const WindowSystem& ws)
{
    ...
    for (EventBindings::iterator it = bindings_.begin(), end = bindings_.end(); it != end; ++it)
    {
        for (devices_map::iterator dit = devices_.begin(), dend = devices_.end(); dit != dend; ++dit)
            it->value.enabled = dit->second->is_event_active(it->value.enabled, it->value.id);
    }
}
