...
template <class Vertex>
bool init_mesh(Mesh& mesh, const std::vector<Vertex>& vertexes, const std::vector<uint32_t>& indexes, Context* context)
{
...
bool init_mesh(Mesh& mesh, uint8_t layout, const MeshExportData& mesh_export_data, 
    ...
    const std::vector<MaterialExportData>& materials_data, const std::vector<MeshPartExportData>& parts_data, Context* context)
{
    init_trace_data(mesh, vertexes, indexes, context);
...
bool load_mesh_version1(Mesh& mesh, std::istream& stream, Context* context)
{
...
bool load_mesh_version2(Mesh& mesh, std::istream& stream, Context* context)
{
    uint8_t layout;
