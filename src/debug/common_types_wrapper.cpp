...
bool transform_get_data(game::Engine& engine, uint8_t* data, size_t& size, size_t offset)
{
    ...
    size_t xforms_number = engine.scene_context().transform_pool.size();
    ASSERT(xforms_number * transform_instance_serialization_data_size() < size, "Invalid serialization buffer size");
    size = xforms_number * transform_instance_serialization_data_size();
